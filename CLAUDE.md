...
| Frontend | React + Vite + react-router | Tabs: Overview (`/`), Map (`/map`), The Fleet (`/fleet`), The Grid (`/grid`), Dispatches (`/dispatches`); plus reactor permalinks (`/reactor/:slug`). `vercel.json` rewrites extensionless routes to index.html so deep links survive refresh |
...
- **Tab theory (placement rule)** — one tab per visitor question: Overview = the argument · Map = places · The Fleet = our own performance · The Grid = nuclear vs. other sources · Dispatches = change over time · Scenarios = what-ifs (future). Place a new feature by this rule; don't bolt it onto whatever page is handy.
